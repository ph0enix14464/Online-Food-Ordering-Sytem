    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult ProductList(string search, int? page, int id = 0)
        {
            ...
            if (id != 0)
            {
                ...
                return View(db.tblItems.Where(p => p.SubMenuId == id).ToList().ToPagedList(page ?? 1, 12));
            }
            else
            {
                if (search != "")
                {
                    return View(db.tblItems.Where(x => x.Description.Contains(search) || x.Title.Contains(search) || search == null).ToList().ToPagedList(page ?? 1, 12));
                }
                else
                {
                    return View(db.tblItems.ToList().ToPagedList(page ?? 1, 12));
                }
